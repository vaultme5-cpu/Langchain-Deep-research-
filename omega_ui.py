...
with st.sidebar:
    ...
    st.info("**Gemini Brain:** " + _env_preview("GEMINI_API_KEY"))
    st.info("**Jina Eyes:** " + _env_preview("JINA_API_KEY"))
    try:
        from open_deep_research import deep_researcher as _dr
        st.caption("Token budget: " + str(int(_dr._RUN_BUDGET["used"])) + " / " + str(int(_dr._RUN_BUDGET["cap"])) + " used")
        _lk = [k for k in list(_dr._BRAIN_HEALTH.keys()) if _dr._brain_is_open(k)]
        st.caption("Locked brains: " + (", ".join(_lk) if _lk else "none"))
    except Exception:
        pass
...
if query:
    ...
    st.session_state.audit_metrics = {}
    st.session_state.last_error = None
    if ("exhausted" in str(st.session_state.last_error or "") or "locked" in str(st.session_state.last_error or "")) and (_ui_time.time() - float(st.session_state.get("last_fail_ts", 0.0)) < 300.0):
        st.warning("Capacity locked: quota exhaustion happened less than 5 minutes ago. Waiting protects your daily tokens. No tokens will be spent now.")
        st.stop()
