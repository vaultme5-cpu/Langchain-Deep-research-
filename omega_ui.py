...
# Make sure Streamlit can import src/open_deep_research
...
st.markdown(
    '<p class="omega-sub">Algorithmic Ruthlessness. Zero-Token Fact-Checking. Groq LPU Supremacy.</p>',
    unsafe_allow_html=True,
)
...
def _env_preview(name: str) -> str:
    ...
    return f"Online ({len(parts)} key{'s' if len(parts) != 1 else ''})"


def _run_graph(query: str):
    # Lazy import: lets the UI start even if the engine import fails.
...
with st.sidebar:
    ...
    st.info(f"**Groq Brain:** {_env_preview('GROQ_API_KEYS')}")
    st.info(f"**Jina Eyes:** {_env_preview('JINA_API_KEY')}")
        ...
        if values:
            ...
            if values.get("research_plan"):
                st.session_state.telemetry.append(
                    f"🗺️ **DAG Topology:** {len(values['research_plan'])} research vectors."
                )

            if values.get("evidence_graph"):
                st.session_state.telemetry.append(
                    f"🕸️ **Evidence Graph:** {len(values['evidence_graph'])} atomic nodes."
                )

            if values.get("virtual_filesystem"):
                st.session_state.telemetry.append(
                    f"📂 **VFS Artifacts:** {len(values['virtual_filesystem'])} heavy payloads secured."
                )

            if values.get("red_team_findings") not in [None, "", "N/A", "Skipped"]:
                st.session_state.telemetry.append("🛡️ **Adversarial Verification** complete.")
...
if st.session_state.telemetry:
    st.markdown(
        f'<div class="telemetry-text">{"<br>".join(st.session_state.telemetry[-20:])}</div>',
        unsafe_allow_html=True,
    )
            ...
            try:
                ...
                if evidence:
                    ...
                    for i, node in enumerate(evidence[:10]):
                        st.markdown(
                            f"- **Fact {i+1}:** {getattr(node, 'claim', 'N/A')} ([Source]({getattr(node, 'url', '#')}))"
                        )
            except Exception:
                pass
