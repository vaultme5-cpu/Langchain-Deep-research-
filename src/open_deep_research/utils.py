...
##########################
# API KEY ROUTING (FIXED FOR GROQ)
##########################

##########################
# OMEGA SUPREMACY: MULTI-KEY GROQ POOL
##########################
...
##########################
# OMEGA PRIME SNIPER TOOLS (Direct Database APIs)
##########################
@tool(description="Search GitHub for open-source repositories. Returns exact repo names, star counts, primary language, and descriptions.")
...
@tool(description="Search HuggingFace for AI models, datasets, and spaces. Returns model names, download counts, and pipeline tags.")
...
@tool(description="Search Hacker News for top stories and discussions. Returns story titles, points, and comment counts.")
...
@tool(description="Search ArXiv for physics, mathematics, and deep computer science pre-prints.")
...
@tool(description="Search PubMed for medical, biological, and clinical trials.")
...
##########################
# HACKER'S SANDBOX & AUDITORS
##########################
@tool(description="Execute Python code to filter data, calculate statistics, parse JSON, or sort lists. Use this to programmatically process search results. Returns standard output. Max 5 seconds execution time.")
def python_repl(code: str) -> str:
    """Secure Sandboxed Execution with Code Review and Timeout."""
    # 1. Security Review (Block dangerous imports and infinite loops)
    dangerous_patterns = ["import os", "import sys", "import subprocess", "import socket", "while True", "eval(", "exec(", "__import__"]
    for pattern in dangerous_patterns:
        if pattern in code:
            return "SECURITY VIOLATION: Code contains forbidden pattern: " + pattern
    ...
    try:
        with concurrent.futures.ThreadPoolExecutor(max_workers=1) as executor:
            ...
            result = future.result(timeout=5.0) # 5 second hard timeout
            return result or "Code executed successfully (no output)."
    except concurrent.futures.TimeoutError:
        return "EXECUTION TIMEOUT: Code took longer than 5 seconds. Please optimize your code or avoid infinite loops."
    except Exception as e:
        return "EXECUTION ERROR: " + str(e)

@tool(description="Audit a URL to verify if a software or API platform is genuinely 100% free or open-source. Returns VERIFIED_FREE, PAID_ENTERPRISE, or UNKNOWN.")
...
##########################
# TOOL ASSEMBLY & EPISTEMIC VERIFICATION
##########################
...
async def get_all_tools(config: RunnableConfig):
    tools = [tool(ResearchComplete), think_tool, github_sniper, huggingface_sniper, hackernews_sniper, wikipedia_search, arxiv_search, pubmed_search, python_repl, audit_pricing, trend_velocity_auditor]
    configurable = Configuration.from_runnable_config(config)
    tools.extend(await get_search_tool(SearchAPI(get_config_value(configurable.search_api))))
    return tools


@tool(description="Search Semantic Scholar for universal academic papers, citation counts, and abstracts.")
...
##########################
# LAYER 7: INTELLIGENT PERSISTENT MEMORY REDUCERS
##########################
def intelligent_memory_reducer(current: Any, new: Any) -> list:
    """Layer 7: Memory Decay, Pruning, and Deduplication for Lessons Learned."""
    if current is None: current = []
    new_items = new if isinstance(new, list) else [new]
    combined = current + new_items
    
    # 1. Conflict Resolution & Deduplication (Fingerprinting)
    unique_memories = []
    seen_fingerprints = set()
    for memory in combined:
        if not isinstance(memory, str): continue
        words = set(re.findall(r'\b\w{5,}\b', memory.lower()))
        fingerprint = "".join(sorted(list(words)[:10]))
        if fingerprint and fingerprint not in seen_fingerprints:
            unique_memories.append(memory)
            seen_fingerprints.add(fingerprint)
            
    # 2. Relevance Pruning (Keep only the last 15 to prevent context bloat)
    return unique_memories[-15:]

def advanced_evidence_graph_reducer(current: Any, new: Any) -> list:
    """Layer 7: Temporal Conflict Resolution & Graph Compression."""
    if current is None: current = []
    ...
    for n in current:
        ...
        fp = "".join(sorted(re.findall(r'\b\w{4,}\b', claim.lower())))
        if fp: claim_map[fp] = n
        
    next_citation = max([getattr(n, 'citation_index', 0) for n in current] + [0]) + 1
    ...
    for node in new_nodes:
        ...
        # Temporal Conflict Resolution (Newer authoritative wins)
        if fp in claim_map:
            existing = claim_map[fp]
            existing_date = getattr(existing, 'date_published', None)
            new_date = getattr(node, 'date_published', None)
            if existing_date and new_date and str(new_date) > str(existing_date):
                claim_map[fp] = node
        else:
    ...
    # 3. Memory Compression (Cap at 100 nodes to prevent context rot)
    final_nodes = list(claim_map.values())
    if len(final_nodes) > 100:
        final_nodes = final_nodes[-100:]
        
    return final_nodes


##########################
# LAYER 8: TREND VELOCITY & REAL-TIME MONITORING
##########################
@tool(description="Calculate the real-time momentum and trend velocity of a topic. Use this to determine if a technology, concept, or event is currently trending up, stable, or dead.")
def trend_velocity_auditor(query: str) -> str:
    """Hits HN and GitHub APIs to calculate 7-day momentum."""
    try:
        import time
        # 1. HackerNews momentum (last 7 days)
        ...
        # 2. GitHub recent activity (last 7 days)
        ...
        return "Velocity audit failed: " + str(e)
...
def filter_and_verify_evidence(evidence_graph: list, temporal_intent: str = "Current") -> list:
    """Context-Aware Epistemic Verification & Temporal Grounding."""
            ...
            if existing_date and new_date:
                if temporal_intent == "Historical":
                    if str(new_date) < str(existing_date):
                        unique_claims[claim_key] = node
                elif temporal_intent == "Timeless":
                    pass  # Dates don't matter, keep the first verified claim
                else:
                    if str(new_date) > str(existing_date):
                        unique_claims[claim_key] = node
