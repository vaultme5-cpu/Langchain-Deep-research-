...
def get_api_key_for_model(model_name, config, last_failed=None):
    ...
    if m.startswith("google_genai:") or m.startswith("gemini"): return os.environ.get("GEMINI_API_KEY", "")
    return None
...
async def get_all_tools(config):
    ...
    tools = [research_complete_tool, think_tool, omega_pdf_ingestor, github_sniper, huggingface_sniper, wikipedia_rest_search, arxiv_search, python_repl, audit_pricing]
