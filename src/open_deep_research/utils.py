...
##########################
# REBUILD STEP 3: The Pricing Auditor (Zero-Cost Constraint Enforcement)
##########################
@tool(description="Audit a URL to verify if a software, API, or LLM platform is genuinely 100% free or open-source. Use this to validate tools against the user's 'Must be free' constraint. Returns 'VERIFIED_FREE', 'PAID_ENTERPRISE', or 'UNKNOWN'.")
def audit_pricing(url: str) -> str:
    """Programmatic pricing auditor. Bypasses LLM hallucination by scanning site text for enterprise/payment traps."""
    try:
        headers = {"User-Agent": "Mozilla/5.0 (Windows NT 10.0; Win64; x64) ProjectOmega/1.0"}
        ...
        # Red Flags (Enterprise / Paid / Freemium traps)
        red_flags = ["contact sales", "book a demo", "enterprise pricing", "credit card required", "upgrade to pro", "request a quote", "talk to an expert", "pricing plans", "enterprise solution"]
        
        # Green Flags (Genuinely Free / Open Source / Playground)
        green_flags = ["open source", "free tier", "free forever", "playground", "100% free", "mit license", "apache license", "huggingface space", "github.com", "chat for free", "no credit card"]
        
        red_count = sum(1 for flag in red_flags if flag in text)
        green_count = sum(1 for flag in green_flags if flag in text)
        
        if green_count > red_count and green_count > 0:
            return "VERIFIED_FREE: " + url + " appears to be genuinely free or open-source based on site text."
        elif red_count > 0 and green_count == 0:
            return "PAID_ENTERPRISE: " + url + " requires payment, enterprise contact, or is a restricted freemium trap. DISCARD this tool if the user requested 100% free."
        else:
            return "UNKNOWN: Could not definitively verify the pricing model for " + url + ". Proceed with caution and look for community verification (e.g., Reddit/GitHub)."
    except Exception as e:
        return "AUDIT_FAILED: Could not access " + url + ". Error: " + str(e)
