...
def _claims_contradict(a, b):
    ...
    if na and nb:
        ...
        for x in na:
            for y in nb:
                if abs(x - y) <= max(0.01, abs(x) * 0.01): matched = True
        if not matched: return True
    if _has_negation(a) != _has_negation(b):
        ta = set(re.findall(r"[a-z0-9_]{4,}", str(a or "").lower()))
        tb = set(re.findall(r"[a-z0-9_]{4,}", str(b or "").lower()))
        if ta and tb and len(ta.intersection(tb)) / max(1, min(len(ta), len(tb))) >= 0.35: return True
    return False

def compute_epistemic_links(nodes):
    """Deterministically links claims using SimHash and Hamming distance."""
    if not nodes: return []
    for idx, node in enumerate(nodes, start=1):
        if getattr(node, "citation_index", 0) <= 0: node.citation_index = idx
        if not getattr(node, "doc_id", ""): node.doc_id = hashlib.sha256(str(getattr(node, "claim", "")).encode("utf-8")).hexdigest()[:16]
    ...
    for i, n1 in enumerate(nodes):
        ...
        for j, n2 in enumerate(nodes):
            ...
            c1, c2 = getattr(n1, "claim", ""), getattr(n2, "claim", "")
            if dist <= CONTRADICTION_DISTANCE and _claims_contradict(c1, c2): candidates.append((dist, target, 1))
            elif dist <= SUPPORT_DISTANCE: candidates.append((dist, target, 0))
        candidates.sort(key=lambda x: (x[0], x[1]))
        s_count, c_count = 0, 0
        for dist, target, kind in candidates:
