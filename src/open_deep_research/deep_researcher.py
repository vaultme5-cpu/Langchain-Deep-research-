...
async def safe_llm_invoke(model, messages, max_attempts=3):
    sem = _get_groq_burst_semaphore(3)
    async with sem:
        last_error = None
    ...
    try:
        ...
        if len(tool_text) > chunk_limit * 2:
            ...
            for c in chunks:
                ...
                try:
                    r = await safe_llm_invoke(sm, msgs)
...
async def final_report_generation(state, config):
    try:
        ...
        rep = await safe_llm_invoke(init_chat_model(model=wc["model"], max_tokens=wc["max_tokens"], api_key=wc["api_key"]), [HumanMessage(content=prompt)])
