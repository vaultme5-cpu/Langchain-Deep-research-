...
from open_deep_research.utils import (
    ...
    get_today_str, is_token_limit_exceeded, remove_up_to_last_ai_message,
    validate_urls, think_tool, compile_search_results,
...
def add_targeted_research_nodes(evidence_graph, research_plan=None):
    plan = list(research_plan or [])
...
async def clarify_with_user(state: AgentState, config: RunnableConfig):
    try:
        ...
        cm = configurable_model.with_structured_output(ClarifyWithUser).with_config(mc)
...
async def write_research_brief(state: AgentState, config: RunnableConfig):
    try:
        configurable = Configuration.from_runnable_config(config)
        mc = {"model": configurable.research_model, "max_tokens": configurable.research_model_max_tokens, "api_key": get_api_key_for_model(configurable.research_model, config), "tags": ["langsmith:nostream"]}
        rm = configurable_model.with_structured_output(ResearchQuestion).with_config(mc)
...
async def supervisor(state: SupervisorState, config: RunnableConfig):
    try:
        configurable = Configuration.from_runnable_config(config)
        mc = {"model": configurable.research_model, "max_tokens": configurable.research_model_max_tokens, "api_key": get_api_key_for_model(configurable.research_model, config), "tags": ["langsmith:nostream"]}
        rm = configurable_model.bind_tools([ConductResearch, ResearchComplete, think_tool]).with_config(mc)
...
async def supervisor_tools(state: SupervisorState, config: RunnableConfig):
    try:
        ...
        if conduct_calls:
            ...
            if vfs_update: update_payload["virtual_filesystem"] = vfs_update
            ...
            if newly_completed: update_payload["completed_nodes"] = list(set(state.get("completed_nodes", [])).union(set(newly_completed)))
...
async def researcher(state: ResearcherState, config: RunnableConfig):
    try:
        ...
        rm = configurable_model.bind_tools(tools).with_config(mc)
...
async def researcher_tools(state: ResearcherState, config: RunnableConfig):
    try:
        ...
        last_msg = r_msgs[-1]
        if not getattr(last_msg, "tool_calls", None): return Command(goto="compress_research")
        tools = await get_all_tools(config)
...
async def compress_research(state: ResearcherState, config: RunnableConfig):
    try:
        ...
        sm = configurable_model.with_structured_output(EvidenceGraphExtraction).with_config(mc)
...
researcher_builder = StateGraph(ResearcherState, output_schema=ResearcherOutputState, config_schema=Configuration)
...
builder = StateGraph(AgentState, input_schema=AgentInputState, config_schema=Configuration)
