...
async def safe_llm_invoke(model, messages, max_attempts=4, brain_name=None):
    ...
    async with sem:
        ...
        for attempt in range(max_attempts):
            try:
                ...
                last_error = e
                err = str(e).lower()
                if brain_name and ("limit: 0" in err or "404" in err or "not found" in err):
                    _brain_open(brain_name, 21600.0, "dead model or zero free quota")
                    raise RuntimeError("[EPISTEMIC FLAG]: " + str(e))
                if brain_name and ("rate limit" in err or "429" in err or "resource_exhausted" in err):
                    _brain_open(brain_name, _parse_retry_seconds(err), "quota locked")
                    raise RuntimeError("[EPISTEMIC FLAG]: " + str(e))
                if "rate limit" in err or "429" in err or "resource_exhausted" in err:
                    new_key = _shield.get_key(last_failed=_shield.get_key())
...
_GEMINI_PROBE_CACHE = {"ts": 0.0, "models": []}

def _probe_gemini_models():
    import httpx, os, time as _t
    now = _t.time()
    if now - _GEMINI_PROBE_CACHE["ts"] < 3600.0 and _GEMINI_PROBE_CACHE["models"]:
        return _GEMINI_PROBE_CACHE["models"]
    key = os.environ.get("GEMINI_API_KEY", "") or os.environ.get("GOOGLE_API_KEY", "")
    if not key: return []
    try:
        r = httpx.get("https://generativelanguage.googleapis.com/v1beta/models", params={"key": key, "pageSize": 100}, timeout=10.0)
        r.raise_for_status()
        ids = []
        for it in r.json().get("models", []):
            nm = str(it.get("name", "")).replace("models/", "")
            low = nm.lower()
            if "flash" in low and not any(b in low for b in ["image", "tts", "veo", "embed", "live", "robot"]):
                ids.append(nm)
        ids.sort(reverse=True)
        if ids:
            _GEMINI_PROBE_CACHE["ts"] = now
            _GEMINI_PROBE_CACHE["models"] = ids[:4]
        return _GEMINI_PROBE_CACHE["models"]
    except Exception as e:
        logging.error("gemini probe failed: " + str(e))
        return _GEMINI_PROBE_CACHE["models"]

def _gemini_ladder(cfg):
    import os
    forced = str(os.environ.get("GEMINI_MODELS", "") or "").strip()
    ladder = ["google_genai:" + m for m in _probe_gemini_models()]
    if forced:
        for m in [x.strip() for x in forced.split(",") if x.strip()]:
            if m not in ladder: ladder.append(m)
    if not ladder:
        for m in str(getattr(cfg, "gemini_models", "") or "").split(","):
            if m.strip() and m.strip() not in ladder: ladder.append(m.strip())
    if not ladder: ladder = [str(getattr(cfg, "gemini_model", "google_genai:gemini-2.0-flash"))]
    return [(m, cfg.gemini_model_max_tokens) for m in ladder]
...
def _parse_retry_seconds(err, default=300.0):
    ...
    m = re.search("retrydelay[^0-9]*([0-9]+)s", err)
    if m: return float(m.group(1))
    return default
...
def _lock_summary():
    ...
    now = _t.time()
    parts = []
    for k, v in list(_BRAIN_HEALTH.items()):
        parts.append(k + " (" + str(int(max(0.0, v[0] - now) // 60)) + "m left)")
...
import os
_RUN_BUDGET = {"used": 0.0, "cap": float(os.environ.get("RUN_TOKEN_BUDGET", "90000"))}
...
def _budget_add(messages, resp):
    try:
        ...
        if um is not None:
            inn = int(getattr(um, "input_tokens", inn) or inn)
            out = int(getattr(um, "output_tokens", out) or out)
        prev = _RUN_BUDGET["used"]
        _RUN_BUDGET["used"] = prev + float(inn + out)
        for th in (0.5, 0.8):
            if prev < th * _RUN_BUDGET["cap"] <= _RUN_BUDGET["used"]:
                logging.error("token budget crossed " + str(int(th * 100)) + "%: " + str(int(_RUN_BUDGET["used"])))
    except Exception:
        pass
def _brain_chain(cfg, kind):
    g = _gemini_ladder(cfg)
    g8 = [(cfg.intake_model, cfg.intake_model_max_tokens)]
    g70 = [(cfg.research_model, cfg.research_model_max_tokens)]
    if _budget_left() < 0.3 * _RUN_BUDGET["cap"]:
        if kind == "report": return g70 + g8 + g
        if kind == "compress": return g8 + g
        return g8 + g70 + g
    if kind == "compress": return g + g8 + [(cfg.compression_model, cfg.compression_model_max_tokens)]
    if kind == "intake": return g8 + g
    if kind == "report": return g70 + g8 + g
    return g70 + g8 + g
...
async def _brain_invoke(cfg, config, kind, messages, structured=None, tools=None):
    ...
    if _budget_left() <= 0.0:
        raise RuntimeError("[EPISTEMIC FLAG]: Run token budget exhausted. Used " + str(int(_RUN_BUDGET["used"])) + " of " + str(int(_RUN_BUDGET["cap"])) + ".")
    last = None
    for name, tok in chain:
        try:
            ...
            return await safe_llm_invoke(m, messages, brain_name=name)
        except Exception as e:
            last = e
            err = str(e).lower()
...
async def researcher_tools(state, config):
    try:
        ...
        for o, t in zip(obs, calls):
            to.append(ToolMessage(content="[UNTRUSTED DATA] " + str(o), name=str(t.get("name", "tool")), tool_call_id=str(t.get("id", "tool"))))
...
async def compress_research(state, config):
    try:
        ...
        if len(tool_text) > chunk_limit * 2:
            ...
            if len(chunks_all) > 3:
                chunks = list(dict.fromkeys([chunks_all[i * (len(chunks_all) - 1) // 3] for i in range(4)]))
            else:
                chunks = chunks_all
            for c in chunks:
...
async def final_report_generation(state, config):
    try:
        ...
        for n in verified:
            ...
            vn.append("[" + str(len(vn) + 1) + "] " + str(getattr(n, "claim", "")) + NL + "Source: " + str(getattr(n, "url", "")) + NL + "Date: " + d)
        ...
        content = getattr(rep, "content", "")
        if not re.search(r"\[\d+\]", content):
            content = content + NL + NL + "Epistemic Note: citation markers were not applied by the model; rely on the numbered Findings above."
        return {"final_report": content, "messages": [rep], "notes": {"type": "override", "value": []}}
    except Exception as e:
