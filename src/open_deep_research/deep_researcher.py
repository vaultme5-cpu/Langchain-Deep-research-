...
def _chunk_messages_for_compression(messages, char_budget=3200, max_messages=4):
    prepared = _expand_messages_for_compression(
        messages,
        max_segment_chars=max(900, char_budget // 2),
    )
    token_budget = max(1000, char_budget // 3)
    chunks = []
    current = []
    current_tokens = 0
    ...
    for msg in prepared:
        ...
        estimated_tokens = _estimate_message_tokens(text) + 20
        if current and (current_tokens + estimated_tokens > token_budget or len(current) >= max_messages):
...
async def _compress_message_chunk(sm, chunk_messages, configurable, depth=0):
    ...
    try:
        ...
        if is_token_limit_exceeded(e, configurable.compression_model) and depth < 5:
            ...
            split_single = _expand_messages_for_compression(chunk_messages, max_segment_chars=800)
            if len(split_single) > len(chunk_messages):
                return await _compress_message_chunk(sm, split_single, configurable, depth + 1)
        raise
