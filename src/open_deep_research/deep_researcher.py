"""Main LangGraph implementation for Project Omega Supremacy."""
import asyncio
import hashlib
import logging
import re
...
async def safe_llm_invoke(model, messages):
    """Pillar 3: LLM Recovery with Groq Pool Rotation and Epistemic Flags."""
...
async def clarify_with_user(state: AgentState, config: RunnableConfig):
    ...
    cm = configurable_model.with_structured_output(ClarifyWithUser).with_retry(
        stop_after_attempt=configurable.max_structured_output_retries
    ).with_config(mc)
    prompt_content = clarify_with_user_instructions.format(
        messages=get_buffer_string(messages), date=get_today_str()
    )
    response = await cm.ainvoke([HumanMessage(content=prompt_content)])
...
async def write_research_brief(state: AgentState, config: RunnableConfig):
    ...
    rm = configurable_model.with_structured_output(ResearchQuestion).with_retry(
        stop_after_attempt=configurable.max_structured_output_retries
    ).with_config(mc)
    ...
    response = await rm.ainvoke([HumanMessage(content=prompt_content)])
...
async def meta_cognitive_router(state: AgentState, config: RunnableConfig):
    ...
    prompt_content = meta_cognitive_router_prompt.format(
    ...
    plan_dicts = []
    if hasattr(response, "research_plan") and response.research_plan:
        plan_dicts = [n.dict() for n in response.research_plan]
...
async def supervisor_tools(state: SupervisorState, config: RunnableConfig):
    ...
    last_msg = sup_msgs[-1]
    if iters > configurable.max_researcher_iterations or not last_msg.tool_calls or any(
        tc["name"] == "ResearchComplete" for tc in last_msg.tool_calls
    ):
        return Command(goto=END, update={
            "notes": get_notes_from_tool_calls(sup_msgs),
            "research_brief": state.get("research_brief", ""),
        })
    all_tool_msgs = []
    update_payload = {"supervisor_messages": []}
    for tc in [t for t in last_msg.tool_calls if t["name"] == "think_tool"]:
        all_tool_msgs.append(ToolMessage(
            content="Reflection recorded: " + tc["args"]["reflection"],
            name="think_tool", tool_call_id=tc["id"],
        ))
    ...
    if conduct_calls:
        ...
        try:
            ...
            for obs, tc in zip(results, allowed):
                ...
                all_tool_msgs.append(ToolMessage(
                    content="ARTIFACT ID: " + art_id + "\nEXECUTIVE SUMMARY:\n" + summary,
                    name=tc["name"], tool_call_id=tc["id"],
                ))
                agg_graph.extend(obs.get("evidence_graph", []))
            for tc in overflow:
                all_tool_msgs.append(ToolMessage(
                    content="Error: Exceeded max concurrent units.",
                    name="ConductResearch", tool_call_id=tc["id"],
                ))
            if artifacts_update:
                update_payload["research_artifacts"] = artifacts_update
            if agg_graph:
            ...
            if raw_concat:
                update_payload["raw_notes"] = [raw_concat]
        except Exception:
            return Command(goto=END, update={
                "notes": get_notes_from_tool_calls(sup_msgs),
                "research_brief": state.get("research_brief", ""),
...
async def researcher(state: ResearcherState, config: RunnableConfig):
    ...
    rm = configurable_model.bind_tools(tools).with_retry(
        stop_after_attempt=configurable.max_structured_output_retries
    ).with_config(mc)
...
async def execute_tool_safely(tool, args, config):
    """Pillar 3: Tool Recovery with Exponential Backoff."""
    for attempt in range(3):
        try:
            ...
            if "rate limit" in err or "429" in err or "timeout" in err or "resource_exhausted" in err:
                ...
                continue
            if attempt == 2:
                return "[TOOL FALLBACK]: " + getattr(tool, "name", "unknown") + " failed."
...
async def researcher_tools(state: ResearcherState, config: RunnableConfig):
    ...
    last_msg = r_msgs[-1]
    if not last_msg.tool_calls and not (
        openai_websearch_called(last_msg) or anthropic_websearch_called(last_msg)
    ):
        return Command(goto="compress_research")
    tools = await get_all_tools(config)
    tools_by_name = {
        t.name if hasattr(t, "name") else t.get("name", "web_search"): t for t in tools
    }
    obs = await asyncio.gather(*[
        execute_tool_safely(tools_by_name[tc["name"]], tc["args"], config)
        for tc in last_msg.tool_calls
    ])
    sniper_tools = ["github_sniper", "huggingface_sniper", "hackernews_sniper", "semantic_scholar_search"]
    compiled_obs = []
    for o, tc in zip(obs, last_msg.tool_calls):
        if isinstance(o, str) and tc["name"] in sniper_tools:
            compiled_obs.append(compile_search_results(tc["name"], o))
        else:
            compiled_obs.append(o)
    tool_outputs = [
        ToolMessage(content=o, name=tc["name"], tool_call_id=tc["id"])
        for o, tc in zip(compiled_obs, last_msg.tool_calls)
    ]
...
async def compress_research(state: ResearcherState, config: RunnableConfig):
    configurable = Configuration.from_runnable_config(config)
    sm = configurable_model.with_config({
    ...
    while attempts < 3:
        try:
            ...
            if is_token_limit_exceeded(e, configurable.research_model):
                r_msgs = remove_up_to_last_ai_message(r_msgs)
    raw_notes = "\n".join([str(m.content) for m in filter_messages(r_msgs, include_types=["tool", "ai"])])
    return {
        ...
        "evidence_graph": [],
        "artifact_id": "error",
        "executive_summary": "Research synthesis failed.",
    }
...
def generate_argus_view(nodes: list) -> str:
    """Pillar 2: Highly compressed topological summary of the Evidence DAG."""
    if not nodes:
    ...
    if foundational:
        ...
        for n in foundational[:5]:
            view += "- [" + str(n.citation_index) + "] " + n.claim + " (Supported by " + str(support_counts[n.citation_index]) + ")\n"
    if contradicted:
        view += "\n**Active Dissent:**\n"
        for n in contradicted[:5]:
            targets = ", ".join([str(c) for c in n.contradicts])
            view += "- [" + str(n.citation_index) + "] " + n.claim + " (Contradicts: " + targets + ")\n"
    if core:
        view += "\n**Peripheral Claims:**\n"
        for n in core:
            view += "- [" + str(n.citation_index) + "] " + n.claim + "\n"
    return view
...
async def adversarial_verification(state: AgentState, config: RunnableConfig):
    ...
    if tier in ["Simple", "Medium"]:
        return Command(goto="final_report_generation", update={
            "red_team_findings": "Skipped",
            "devils_advocate_critique": "Skipped",
            "consensus_report": state.get("master_synthesis", ""),
    ...
    async def run_red():
        ...
        try:
            return (await safe_llm_invoke(configurable_model.with_config(mc), [HumanMessage(content=red_team_prompt.format(brief=brief, findings=trunc, master_synthesis=""))])).content
        except Exception:
            return "[EPISTEMIC FLAG]: Red Team skipped."

    async def run_dev():
        mc = {"model": configurable.research_model, "max_tokens": 2048, "api_key": get_api_key_for_model(configurable.research_model, config), "tags": ["langsmith:nostream"]}
        try:
            return (await safe_llm_invoke(configurable_model.with_config(mc), [HumanMessage(content=devils_advocate_prompt.format(brief=brief, findings=trunc, master_synthesis=""))])).content
        except Exception:
            return "[EPISTEMIC FLAG]: Devil Advocate skipped."
    ...
    try:
        cons_res = await safe_llm_invoke(
            configurable_model.with_config(mc),
            [HumanMessage(content=consensus_builder_prompt.format(brief=brief, findings=trunc, red_team_findings=red_r, devils_advocate_critique=dev_r))],
        )
        ...
        avg_conf = 0.5
    return Command(goto="final_report_generation", update={
        "red_team_findings": red_r,
        "devils_advocate_critique": dev_r,
        "consensus_report": cons_text,
...
async def final_report_generation(state: AgentState, config: RunnableConfig):
    ...
    if raw_ev:
        ...
        for n in verified:
            ...
            v_notes.append("Fact: " + str(getattr(n, "claim", "")) + "\nSource: " + str(getattr(n, "title", "")) + " (" + str(getattr(n, "url", "")) + ")\nDate: " + d)
    ...
    while retries <= 3:
        try:
            prompt = final_report_generation_prompt.format(
                ...
                findings=findings,
                date=get_today_str(),
                master_synthesis=state.get("master_synthesis", "Standard synthesis."),
            ...
            if is_token_limit_exceeded(e, configurable.final_report_model):
                ...
                return {"final_report": "Error: " + str(e), "messages": [AIMessage(content="Failed")], **cleared}
...
async def meta_learning_node(state: AgentState, config: RunnableConfig):
    ...
    res = await configurable_model.with_config(mc).ainvoke(
        [HumanMessage(content=meta_learning_prompt.format(confidence_score=conf, iterations=iters))]
    )
