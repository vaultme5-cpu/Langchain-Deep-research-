"""Omega Supremacy Engine (Groq Supremacy, Quality-First)."""
...
from open_deep_research.state import (compute_epistemic_links, AgentInputState, AgentState, ClarifyWithUser, ConductResearch, EvidenceGraphExtraction, ResearchComplete, ResearcherOutputState, ResearcherState, ResearchQuestion, RouterDecision, SupervisorState)
...
def _brain_chain(cfg, kind):
    ...
    if _budget_left() < 0.3 * _RUN_BUDGET["cap"]:
        if kind == "report": return g70 + g8
        return g8 + g70
    if kind == "compress": return g70 + g8
    if kind == "intake": return g8 + g70
    if kind == "report": return g70 + g8
    return g70 + g8
...
async def _brain_invoke(cfg, config, kind, messages, structured=None, tools=None):
    ...
    for name, tok in chain:
        if _brain_is_open(name):
            logging.error("brain skipped (locked): " + name)
            continue
        try:
...
async def researcher(state, config):
    try:
        ...
        rm = init_chat_model(model=cfg.research_model, max_tokens=cfg.research_model_max_tokens, api_key=get_api_key_for_model(cfg.research_model, config)).bind_tools(tools)
...
async def supervisor_tools(state, config):
    try:
        ...
        nc = [str((t.get("args", {}) or {}).get("node_id")) for t in allowed if (t.get("args", {}) or {}).get("node_id")]
...
async def final_report_generation(state, config):
    try:
        ...
        if not re.search(r"\[\d+\]", content): content = content + NL + NL + "Epistemic Note: citation markers were not applied by the model; rely on the numbered Findings above."
