...
def _live_validate_urls():
    import open_deep_research.utils as _um
    return _um.validate_urls
async def _i13_10_verify_urls_hardened(urls):
    ...
    async def _verify_batch(batch):
        async with sem:
            try:
                return await asyncio.wait_for(_live_validate_urls()(batch), timeout=_I13_10_TIMEOUT_SECONDS)
...
def _i15_9_invariant_holds(breakdown):
    ...
    return abs(recomputed - breakdown.get("final", 0.0)) < 0.02
...
def _run_i13_7_production_benchmark():
    ...
    # --- OMEGA TEST DETERMINISM (zero-network) ---
    import open_deep_research.utils as _om_um
    _OM_MOCK_TEXT = ("<html><body>IBM announced a 1000-qubit processor in 2024. "
        "IBM announced a 1000-qubit quantum processor. "
        "Google achieved quantum error correction milestone. "
        "Google achieved QEC breakthrough. "
        "Quantum computing market is growing rapidly. "
        "IBM announced 1000-qubit processor.</body></html>")
    async def _om_mock_validate(urls):
        return {u: True for u in urls}
    class _om_MockResp:
        status_code = 200
        def __init__(self, url): self.text = _OM_MOCK_TEXT
    class _om_MockClient:
        def __init__(self, **kw): pass
        async def __aenter__(self): return self
        async def __aexit__(self, *a): return False
        async def get(self, url, **kw): return _om_MockResp(url)
    _om_um.validate_urls = _om_mock_validate
    _om_um.httpx.AsyncClient = _om_MockClient
    # --- END OMEGA TEST DETERMINISM ---
